...
def phislice_for_eposition(ex):
    if not os.path.exists(directory):
        os.makedirs(directory)
    phislice = my_cavity.field_map_2d_phislice(rel_modes, phiprobe, ex, electron_dipole, f, plotz=False)
    np.savetxt('{2}/f25_phislice_er{0}_ez{1}_zhalf_Er.txt'.format(ex[0],ex[2], directory),phislice[0])
    np.savetxt('{2}/f25_phislice_er{0}_ez{1}_zhalf_Ephi.txt'.format(ex[0],ex[2], directory),phislice[1])
    plt.savefig('{2}/f26_phislice_er{0}_ez{1}_phi0_zhalf.png'.format(ex[0], ex[2], directory), bbox_inches = 'tight', dpi=resolution)
    plt.close()

if __name__ == '__main__':
    pool = Pool(processes=2)
    pool.map(phislice_for_eposition, electron_position)
